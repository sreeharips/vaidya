...
const PACKAGE_TYPES = ["panchakarma", "wellness", "therapeutic", "rejuvenation"] as const;
...
export default function PackagesPage() {
  ...
  const [editing, setEditing] = useState<Partial<Package>>(EMPTY_PACKAGE);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await updatePackage(editId, editing);
      } else {
  ...
  const updateField = (partial: Partial<Package>) => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-2xl text-slate">Packages</h1>
        <button
          onClick={openAdd}
          ...
          <table className="w-full text-sm font-sans">
            ...
            <tbody className="divide-y divide-cream2">
              {packages.length === 0 ? (
                ...
                packages.map((pkg) => (
                  <tr key={pkg.id} className="hover:bg-cream/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate">{pkg.name_display_en || pkg.name}</div>
                      {pkg.max_guests_per_slot > 1 && (
                        <div className="text-xs text-muted mt-0.5">
                          Up to {pkg.max_guests_per_slot} guests
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {(pkg.wellness_categories || []).slice(0, 2).map((cat) => (
                          <span key={cat} className="px-1.5 py-0.5 bg-gold-lt text-gold rounded text-xs">
                            {formatCategory(cat)}
                          </span>
                        ))}
                        {(pkg.wellness_categories || []).length > 2 && (
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className={`px-2 py-0.5 rounded text-xs font-medium cursor-pointer transition-colors ${
                          pkg.is_active ? "bg-forest-lt text-forest" : "bg-cream2 text-muted"
                        }`}
                      >
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full max-w-lg bg-white shadow-xl overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              {/* Name */}
              <div>
                <label className="block text-sm font-sans text-slate mb-1">Package Name</label>
                <input
                  type="text"
                  value={editing.name || ""}
              ...
              {/* Package type */}
              <div>
                <label className="block text-sm font-sans text-slate mb-1">Package Type</label>
                <select
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {saving ? "Saving..." : editId ? "Update Package" : "Add Package"}
                </button>
                <button
                  onClick={() => setPanelOpen(false)}
