...
export async function generateMetadata({ params: { lang } }: PageProps): Promise<Metadata> {
  return {
    title: 'Ayurvedic Doctors in Kerala — Browse Vaidyas | Vaidya',
    description: 'Find credentialed BAMS/MD Ayurvedic doctors in Kerala. Filter by specialisation, district, language, experience, and Prakriti affinity.',
    alternates: { canonical: `https://vaidya.health/${lang}/doctors` },
  }
}

async function fetchDoctors(params: URLSearchParams): Promise<{ items: DoctorSummary[]; total: number }> {
  try {
    const res = await fetch(`${API_BASE}/api/doctors?${params.toString()}`, { next: { revalidate: 120 } })
...
export default async function DoctorsPage({ params: { lang }, searchParams }: PageProps) {
  ...
  const yearsExpMin    = searchParams.years_exp_min
  ...
  const { items: doctors, total } = await fetchDoctors(qs)

  const totalPages    = Math.ceil(total / PAGE_SIZE)
  const activeFilters = [tier, specialisation, district, language, prakriti, budgetMax, ratingMin, gender, yearsExpMin].filter(Boolean).length
  ...
  const buildUrl = (overrides: Record<string, string | undefined>) => {
    ...
    return `/${lang}/doctors?${p.toString()}`
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 48px 64px', display: 'flex', gap: 32, alignItems: 'flex-start' }}>
        ...
        <aside style={{ width: 240, flexShrink: 0, position: 'sticky', top: 24 }}>
          <ListingFilterBar
            basePath={`/${lang}/doctors`}
            current={{ tier, specialisation, district, language, prakriti, budgetMax, ratingMin, gender, yearsExpMin }}
            options={{
              districts: DISTRICTS,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {activeFilters > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              ...
              <a href={`/${lang}/doctors`} style={{ fontSize: 12, color: 'var(--forest)', textDecoration: 'underline' }}>
                Clear all
              </a>
            </div>
          )}
          ...
          {doctors.length > 0 ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16, alignItems: 'stretch' }}>
              {doctors.map((doctor) => (
                <HomeDoctorCard key={doctor.id} doctor={doctor} lang={lang} />
              ))}
            </div>
          ) : (
            <div style={{ padding: '60px 24px', textAlign: 'center', border: '1px dashed var(--border2)', borderRadius: 'var(--r-md)', color: 'var(--muted)' }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>✦</div>
              <div style={{ fontFamily: 'var(--serif)', fontSize: 18, color: 'var(--forest)', marginBottom: 8 }}>No doctors found</div>
              <div style={{ fontSize: 13 }}>Try adjusting your filters.</div>
              <a href={`/${lang}/doctors`} style={{ display: 'inline-block', marginTop: 16, fontSize: 13, color: 'var(--forest)', textDecoration: 'underline' }}>
                Clear all filters
              </a>
            </div>
          )}
