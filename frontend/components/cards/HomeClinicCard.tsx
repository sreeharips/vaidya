...
export default function HomeClinicCard({
  ...
  return (
    <Link href={`/${lang}/clinics/${clinic.slug}`} style={{ textDecoration: 'none', display: 'flex', height: '100%' }}>
      ...
      >
        {/* Cover photo */}
        <div style={{ height: imgH, background: 'linear-gradient(135deg, var(--forest-lt) 0%, var(--cream2) 100%)', position: 'relative', flexShrink: 0, overflow: 'hidden' }}>
          {clinic.photos?.[0] ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={clinic.photos[0]} alt={clinic.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ) : (
            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontFamily: 'var(--serif)', fontSize: compact ? 40 : 56, color: 'var(--forest2)', opacity: 0.18 }}>✦</span>
            </div>
          )}
          <span style={{ position: 'absolute', top: compact ? 8 : 12, left: compact ? 8 : 12, background: tierBg, color: '#fff', fontSize: compact ? 9 : 10, fontWeight: 600, padding: compact ? '3px 8px' : '4px 10px', borderRadius: 99, letterSpacing: '0.04em' }}>
            {tierLabel}
          </span>
          {clinic.rating && (
            <span style={{ position: 'absolute', top: compact ? 8 : 12, right: compact ? 8 : 12, background: 'rgba(0,0,0,0.6)', color: '#fff', fontSize: compact ? 11 : 12, fontWeight: 600, padding: compact ? '3px 8px' : '4px 10px', borderRadius: 99, display: 'flex', alignItems: 'center', gap: 3 }}>
        ...
        {/* Body */}
        <div style={{ padding: compact ? '12px 14px 12px' : '20px 20px 16px', flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontFamily: 'var(--serif)', fontSize: compact ? 17 : 20, fontWeight: 500, color: 'var(--forest)', lineHeight: 1.25, marginBottom: compact ? 2 : 4 }}>
            {clinic.name}
          </div>
          {clinic.district && (
            <div style={{ fontSize: compact ? 11 : 12, color: 'var(--muted)', marginBottom: compact ? 6 : 10, display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {(clinic.wellness_categories ?? []).length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: compact ? 4 : 5, marginBottom: compact ? 8 : 12 }}>
          ...
          <div style={{ display: 'flex', gap: compact ? 8 : 12, marginBottom: compact ? 8 : 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            {clinic.cheapest_price != null ? (
              <div>
                <span style={{ fontFamily: 'var(--serif)', fontSize: compact ? 16 : 18, color: 'var(--forest)', fontWeight: 500 }}>${clinic.cheapest_price.toLocaleString()}</span>
            ...
            <span style={{ fontSize: compact ? 12 : 13, fontWeight: 600, color: 'var(--forest)', background: 'var(--forest-lt)', padding: compact ? '6px 12px' : '7px 16px', borderRadius: 99 }}>
              View clinic →
            </span>
          </div>
        </div>
